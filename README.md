...
Weekday cadence, in order: chat CLI → RAG → tokenizer → tool agent → prompt lab → embeddings → **vision caption loop (shipped)** → memory → shop-skill explainer. Live demos stay up front on craft repos.
